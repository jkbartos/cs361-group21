...
function continueDestination() {
 var app_key = "AIzaSyCvZRbzXevwNoq34qgmGYBAx-wFNllQLUc";  //cprumsey21 google API Key  cost $5/1000 requests
    ...
    window.location = '/destinations/set/confirmed/?veh_id=' + encodeURI(veh_id) + '&app_key=' + encodeURI(app_key) + '&origin_lat=' + encodeURI(origin_lat) + '&origin_lon=' + encodeURI(origin_lon) + '&dest_addr=' + encodeURI(dest_addr) + '&dest_street=' + encodeURI(dest_street) + '&dest_city=' + encodeURI(dest_city) + '&dest_state=' + encodeURI(dest_state)
}
